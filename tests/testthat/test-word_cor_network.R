context("Test Word Correlation Network")
...
test_that("no filters shows all records", {
  expect_equal(dim(sentiment140_test)[1], dim(word_cor_network(sentiment140_test)$raw)[1])
})

test_that("all filters work", {
  expect_gte(dim(word_cor_network(sentiment140_test,
                                  start_date_time = as_datetime("2009-04-01"),
                                  end_date_time = as_datetime("2009-06-14"))$raw)[1], 1)
})


test_that("should fail for non-date as date arguments", {
  expect_error(word_cor_network(sentiment140_test,
...
test_that("empty keyword list should give full data frame", {
  result <- word_cor_network(sentiment140_test,
                             keyword_list=list())
  expect_equal(dim(result$raw)[1], dim(sentiment140_test)[1])
})
...
test_that("should give valid results", {
  ...
  expect_true(is.data.frame(result$word_cors))
})
