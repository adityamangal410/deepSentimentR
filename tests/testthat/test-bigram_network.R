...
test_that("no filters shows all records", {
  expect_equal(dim(test_data)[1], dim(bigram_network(test_data)$raw)[1])
})

test_that("user filter works", {
  expect_equal(2, dim(bigram_network(test_data, user_list = c("test_user1"))$raw)[1])
})

test_that("date filter works", {
  expect_equal(2, dim(bigram_network(test_data,
                                     start_date_time = as_datetime("2009-03-31"),
                                     end_date_time = as_datetime("2009-04-06"))$raw)[1])
})

test_that("keyword filter works", {
  expect_equal(2, dim(bigram_network(test_data, keyword_list = c("user1"))$raw)[1])
})

test_that("all filters work", {
  expect_gte(dim(bigram_network(test_data,
                                user_list = c("test_user1"))$raw)[1], 1)
})


test_that("should fail for non-date as date arguments", {
  expect_error(bigram_network(test_data,
...
test_that("empty keyword list should give full data frame", {
  result <- bigram_network(test_data,
                           keyword_list=list())
...
test_that("should give valid results", {
  ...
  expect_true(is.data.frame(result$bigram_counts))
  expect_equal(dim(result$bigram_counts)[1], 3)
})
