...
John finds the following "Twitter Airline Sentiment" dataset on - [kaggle](https://www.kaggle.com/crowdflower/twitter-airline-sentiment/version/2)
...
```{r eval=FALSE}
airlines = readr::read_csv("~/Downloads/twitter-airline-sentiment/Tweets.csv", 
                           col_types = readr::cols(.default = readr::col_character()))

airlines %>% 
  mutate(airline_sentiment_confidence = as.numeric(airline_sentiment_confidence)) %>% 
  filter(airline_sentiment_confidence >= 0.8,
         airline_sentiment != "neutral",
         !is.na(user_timezone),
         str_detect(user_timezone, "US & Canada")) %>% 
  select(airline_sentiment, tweet_id, tweet_created, airline, name, text)  %>% 
  mutate(tweet_created = lubridate::as_datetime(stringr::str_replace(tweet_created, " -0800", ""), 
...
airlines %>% 
  readr::write_csv("~/Downloads/twitter-airline-sentiment/twitter_airline_sentiment.csv")
```
...
```{r}
airlines <- readr::read_csv("~/Downloads/twitter-airline-sentiment/twitter_airline_sentiment.csv")
...
deepSentimentR::train_lstm_with_glove(data = airlines,
                                      ...
                                      model_save_path = "~/Downloads/twitter-airline-sentiment/train_glove_lstm.rds")
deepSentimentR::train_conv1d_with_glove(data = airlines,
                                        glove_file_path = "~/Downloads/glove.6B.100d.txt",
                                        model_save_path = "~/Downloads/twitter-airline-sentiment/train_glove_conv_1d.rds")
```
...
deepSentimentR::train_lstm(data = airlines,
                           model_save_path = "~/Downloads/twitter-airline-sentiment/train_no_glove_lstm.rds")
deepSentimentR::train_conv_1d(data = airlines,
                              model_save_path = "~/Downloads/twitter-airline-sentiment/train_no_glove_conv_1d.rds")
```
...
lstm_no_glove_results <- deepSentimentR::predict_polarity_keras(data = airlines,
                                                                model_load_path = "~/Downloads/twitter-airline-sentiment/train_no_glove_lstm.rds")
lstm_glove_results <- deepSentimentR::predict_polarity_keras(data = airlines,
                                     model_load_path = "~/Downloads/twitter-airline-sentiment/train_glove_lstm.rds")
conv_1d_no_glove_results <- deepSentimentR::predict_polarity_keras(data = airlines,
                                     model_load_path = "~/Downloads/twitter-airline-sentiment/train_no_glove_conv_1d.rds")
conv_1d_glove_results <- deepSentimentR::predict_polarity_keras(data = airlines,
                                     model_load_path = "~/Downloads/twitter-airline-sentiment/train_glove_conv_1d.rds")
```
...
As we can see above, the 1-dimensional Convolutional Network model with Glove Embeddings gives the best prediction results for our airlines dataset with an accuracy of about 79.93%. 

Lets make predictions using the pre-trained models provided in the deepSentimentR package. 
